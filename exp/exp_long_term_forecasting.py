                    ...
                    if self.args.auxi_lambda:
                        ...
                        if self.args.auxi_mode == "fft":
                            ...
                            if self.args.auxi_type == 'complex':
                                # 计算傅里叶变换
                                fft_outputs = torch.fft.rfft(outputs, dim=1)
                                fft_batch_y = torch.fft.rfft(batch_y, dim=1)

                                # 计算频率
                                freq = torch.fft.rfftfreq(outputs.size(1))
                                muchang=len(freq)
                                fft_outputsbiao=torch.abs(fft_outputs/muchang)
                                fft_batch_ybiao=torch.abs(fft_batch_y/muchang)
                                mask=int(muchang*0.1)
                                mask1=int(muchang*0.2)
                                mask2=int(muchang*0.6)
                                for i in range(1, fft_outputs.shape[1] - 1):
                                    local_max_mask = (fft_batch_ybiao[:, i] > fft_batch_ybiao[:, i-1]) & (fft_batch_ybiao[:, i] > fft_batch_ybiao[:, i+1])
                                for i in range(1, fft_outputs.shape[1] - 1): 
                                    fft_outputs[:, i][local_max_mask] *= 7.825
                                    fft_batch_y[:, i][local_max_mask] *= 7.825
                                    #if mask1 < i <= mask2:
                                        #fft_outputs[:, i][local_max_mask] *= 10
                                        #fft_batch_y[:, i][local_max_mask] *= 10
                                    #elif i > mask2:
                                        #fft_outputs[:, i][local_max_mask] *= 20
                                        #fft_batch_y[:, i][local_max_mask] *= 20

                                # 计算误差
                                loss_auxi =fft_outputs - fft_batch_y
                                ...
                                fft_batch_ybiao=torch.abs(fft_batch_y/muchang)
                                mask=int(muchang*0.1)
                                mask1=int(muchang*0.3)
                                mask2=int(muchang*0.6)
                                for i in range(1, fft_outputs.shape[1] - 1):
                                    local_max_mask = (fft_batch_ybiao[:, i] > fft_batch_ybiao[:, i-1]) & (fft_batch_ybiao[:, i] > fft_batch_ybiao[:, i+1])
                                    if mask < i <= mask1:
                                        fft_outputs[:, i][local_max_mask] *= 2
                                        fft_batch_y[:, i][local_max_mask] *= 2
                                    elif mask1 < i <= mask2:
                                        fft_outputs[:, i][local_max_mask] *= 10
                                        fft_batch_y[:, i][local_max_mask] *= 10
                                    elif i > mask2:
                                        fft_outputs[:, i][local_max_mask] *= 40
                                        fft_batch_y[:, i][local_max_mask] *= 40
                                loss_auxi_mag = (fft_outputs - fft_batch_y).abs()
                                loss_auxi_phase = (fft_outputs - fft_batch_y).angle()
